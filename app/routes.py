...
@login_required
def translate_text():
    return jsonify({'text': translate(request.form['text'], request.form['source_language'], request.form['dest_language'][0:2])})
...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        # _()函数来自于babel，勇于标记需要翻译的文本
        flash('Your post is now live!')
        return redirect(url_for('index'))
    page = request.args.get('page', 1, type=int)
    # 从flask-SQLALchemy 3.0 开始，所有paginate的参数都必须是keyword like （page=''）
...
def explore():
    ...
    return render_template('index.html', title='Expore', posts=posts.items, next_url=next_url, prev_url=prev_url)
